...
### 2.8 Xác định DRDB Primary node <a name="2.8" />
...
- Tạo sự ràng buộc giữa HAproxy và VirtIP

Chúng ta cấu hình 2 tài nguyên này ràng buộc với nhau.

```
[root@node1 ~]# pcs constraint colocation add VirtIP HAproxy INFINITY
...
**Giải thích:**  Khi start: `VirtIP` sẽ khởi động đầu tiên để sinh ra 1 Virtual IP cho `HAproxy` lắng nghe.
...
[root@node1 ~]# pcs cluster cib-push fs_cfg
CIB updated
[root@hoang ~]# pcs status resources
 VirtIP (ocf::heartbeat:IPaddr2):       Started node1
 HAproxy        (systemd:haproxy):      Started node1
 Master/Slave Set: DrbdDataClone [DrbdData]
     Masters: [ node1 ]
